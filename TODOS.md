...
**Why:** Edge on Mac/Linux does not support IE mode. Users on these platforms who install the extension get no value and may leave 1-star reviews.
**Pros:** Prevents bad reviews, improves user trust.
**Cons:** Minimal — same guard logic as browser detection.
**Context:** Use `navigator.userAgentData.platform` (Chrome/Edge 90+) or fall back to `navigator.platform`. Check for 'Win' prefix. Show informative message with link to Windows Edge download if relevant.
**Effort:** S (human: ~30 min / CC: ~5 min)
**Depends on:** Nothing — can be added at any time
